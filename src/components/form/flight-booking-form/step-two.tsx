...
import { DateTimeInputField } from "@/components/ui/DatePicker";
...
const StepTwo: React.FC = () => {
  ...
  return (
    <form className="space-y-6 p-4 bg-gray-100 " onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col md:flex-row gap-4 p-2 border border-gray-100 rounded-md">
          ...
          <div>
            <Dropdown
              options={OPTIONS}
              label="Cabin Class"
              name="stepTwo.cabinClass"
              value={formData.cabinClass}
              onChange={(val) =>
                handleInputChange({
                  target: { name: "stepTwo.cabinClass", value: val },
                })
              }
            />
          </div>
          <InputField
            className="bg-white"
            label="Airline No"
            name="stepTwo.airlineNo"
            value={formData.airlineNo}
            onChange={handleInputChange}
          />
        </div>
      </section>
        ...
        <section className="space-y-4  pt-4 mt-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 p-2 border border-gray-100 rounded-md bg-gray-50">
            ...
            <InputField
              ...
              required
              label="From"
              name="stepTwo.arrivalflights.0.returnFrom"
              value={formData.returnFrom}
              onChange={handleInputChange}
            />
            <InputField
              className="bg-white"
              required
              label="To"
              name="stepTwo.arrivalflights.0.returnTo"
            ...
            <InputField
              ...
              required
              label="Airline"
              name="stepTwo.arrivalflights.0.returnAirline"
            ...
            <div>
              <Dropdown
                options={OPTIONS}
                label="Cabin Class"
                name="stepTwo.arrivalflights.0.cabinClass"
                value={formData.returnCabinClass}
                onChange={(val) =>
                  handleInputChange({
                    target: {
                      name: "stepTwo.arrivalflights.0.cabinClass",
            ...
            <InputField
              className="bg-white"
              label="Airline No"
              name="stepTwo.arrivalflights.0.returnAirlineNo"
              value={formData.returnAirlineNo}
              onChange={handleInputChange}
            />
          </div>
        </section>
      ...
      <Button className="w-full mt-auto" variant="primary" type="submit">
        Procced to Payment
      </Button>
    </form>
  );
